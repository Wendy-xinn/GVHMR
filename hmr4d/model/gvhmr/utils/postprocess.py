...
@autocast(enabled=False)
def pp_static_joint_ego(outputs, endecoder: EnDecoder):
    """Ego 头的 post-processing：使用 ego 的输出"""
    # Global FK (使用 ego 的输出)
    pred_w_j3d = endecoder.fk_v2(**outputs["pred_smpl_params_global_ego"])
    L = pred_w_j3d.shape[1]
    joint_ids = [7, 10, 8, 11, 20, 21]  # [L_Ankle, L_foot, R_Ankle, R_foot, L_wrist, R_wrist]
    pred_j3d_static = pred_w_j3d.clone()[:, :, joint_ids]  # (B, L, J, 3)
    ...
    static_conf_logits = outputs["static_conf_logits_ego"][:, :-1].clone()
    static_label_ = static_conf_logits > 0  # (B, L-1, J) # avoid non-contact frame
    static_conf_logits = static_conf_logits.float() - (~static_label_ * 1e6)  # fp16 cannot go through softmax
    is_static = static_label_.sum(dim=-1) > 0  # (B, L-1)
    ...
    # Overwrite results:
    if False:  # for-loop
        post_w_transl = outputs["pred_smpl_params_global_ego"]["transl"].clone()  # (B, L, 3)
        for i in range(1, L):
            post_w_transl[:, i:] -= pred_disp[:, i - 1 : i]
    else:  # vectorized
        pred_w_transl = outputs["pred_smpl_params_global_ego"]["transl"].clone()  # (B, L, 3)
        pred_w_disp = pred_w_transl[:, 1:] - pred_w_transl[:, :-1]  # (B, L-1, 3)
        pred_w_disp_new = pred_w_disp - pred_disp
        post_w_transl = torch.cumsum(torch.cat([pred_w_transl[:, :1], pred_w_disp_new], dim=1), dim=1)
        post_w_transl[..., 0] = gaussian_smooth(post_w_transl[..., 0], dim=-1)
...
@autocast(enabled=False)
def pp_static_joint_cam_ego(outputs, endecoder: EnDecoder):
    """Ego 头的 post-processing（带相机先验）：使用 ego 的输出"""
    # input (使用 ego 的输出)
    pred_smpl_params_incam = outputs["pred_smpl_params_incam_ego"].copy()
    pred_smpl_params_global = outputs["pred_smpl_params_global_ego"]
    static_conf_logits = outputs["static_conf_logits_ego"].clone()[:, :-1]  # (B, L-1, J)
    joint_ids = [7, 10, 8, 11, 20, 21]  # [L_Ankle, L_foot, R_Ankle, R_foot, L_wrist, R_wrist]
